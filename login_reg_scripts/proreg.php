		...
		)
		{
			...
			if(!empty($firstname) && !empty($lastname) && !empty($password) && !empty($email) && !empty($gender) && !empty($dob) && !empty($month) && !empty($year) && !empty($dept) && !empty($year2) &&!empty($password1) /*&& !empty($response)*/)
			{
				...
				if($password==$password1)
				{
					...
					{
						$get=mysqli_query($con,"SELECT * FROM login ORDER BY id DESC LIMIT 1");
						$got=mysqli_fetch_array($get);
						$curr_id=$got['id'];
						$next_id=60003160000+$curr_id+1;
						$query="INSERT INTO login VALUES('','".mysqli_real_escape_string($con,$password)."','".mysqli_real_escape_string($con,$firstname)."','".mysqli_real_escape_string($con,$lastname)."','".mysqli_real_escape_string($con,$email)."','".mysqli_real_escape_string($con,$gender)."','".mysqli_real_escape_string($con,$dob)."','".mysqli_real_escape_string($con,$date1)."','".mysqli_real_escape_string($con,$year2)."','".mysqli_real_escape_string($con,$dept)."','','','','','.$ip.','".mysqli_real_escape_string($con,$date1)."','".$next_id."','')";
