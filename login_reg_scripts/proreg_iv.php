	...
	if(
		...
		)
		{
			...
			if(!empty($firstname) && !empty($lastname) && !empty($password) && !empty($email) )
			{
				...
				{
					$query="SELECT * FROM invig WHERE email='".mysqli_real_escape_string($con,$email) ."'";
					...
					{
						$get=mysqli_query($con,"SELECT * FROM invig ORDER BY id DESC LIMIT 1");
						$got=mysqli_fetch_array($get);
						$curr_id=$got['id'];
						$next_id=70003160000+$curr_id+1;
						$query="INSERT INTO invig VALUES('','".mysqli_real_escape_string($con,$firstname)."','".mysqli_real_escape_string($con,$lastname)."','".$next_id."','".mysqli_real_escape_string($con,$password)."','".mysqli_real_escape_string($con,$email)."')";
